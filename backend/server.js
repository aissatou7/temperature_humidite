...
mongoose
  .connect(
    "mongodb+srv://aissatou7:766021841Fall@cluster0.wayru7i.mongodb.net/test",
    ...
    Data.findOne({ date: datei }, (err, data) => {
      if (err) {
        ...
        Data.updateOne(
          ...
          {
            $set: {
              temperature12h: temperer,
              humidite12h: humidy,
              temperatureM: temperatureM1,
              humiditeM: humiditeM1,
            },
          },
          (err, data) => {
            if (err) {
  ...
  if (heur == "13" && min == "47" && sec == "35") {
    ...
    Data.findOne({ date: datei }, (err, data) => {
      if (err) {
        ...
        temperatureSomme2 =
          data.temperature8h + data.temperature12h + parseFloat(temperer);
        ...
        Data.updateOne(
          ...
          {
            $set: {
              temperature18h: temperer,
              humidite18h: humidy,
              tempereratureM: temperatureM2,
              humiditeM: humiditeM2,
            },
          },
          (err, data) => {
            if (err) {
